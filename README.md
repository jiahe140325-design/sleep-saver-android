...
1. 前往 [拯救睡眠 1.2.2-dev 发布页](https://github.com/jiahe140325-design/sleep-saver-android/releases/tag/v1.2.2-dev)，展开 `Assets`，下载 `SleepSaver-1.2.2-dev.apk`。不要下载 `Source code` 压缩包。
...
- 如果微信把文件保存成类似 `SleepSaver-1.2.2-dev.apk.2`，请先在系统「文件管理」中将末尾的 `.2` 删除，确保文件名以 `.apk` 结尾后再打开。
